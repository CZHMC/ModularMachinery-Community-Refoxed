...
public final class SmartInterfaceScreen extends AbstractContainerScreen<SmartInterfaceMenu> {
    ...
    protected void init() {
        ...
        save = addRenderableWidget(new StyledButton(leftPos + SAVE_X, topPos + layout.inputY() - 1,
                SAVE_WIDTH, SAVE_HEIGHT, Component.translatable("mmcr.smart_interface.save"), button -> {
            sendValue();
            button.setFocused(false);
        }));
        previous = addRenderableWidget(new StyledButton(leftPos + layout.previousX(), topPos + layout.navigationY(),
                NAVIGATION_WIDTH, NAVIGATION_HEIGHT, Component.translatable("mmcr.smart_interface.previous"), button -> {
            select(showing - 1);
            button.setFocused(false);
        }));
        next = addRenderableWidget(new StyledButton(leftPos + layout.nextX(), topPos + layout.navigationY(),
                NAVIGATION_WIDTH, NAVIGATION_HEIGHT, Component.translatable("mmcr.smart_interface.next"), button -> {
            select(showing + 1);
            button.setFocused(false);
        }));
        updateWidgets();
